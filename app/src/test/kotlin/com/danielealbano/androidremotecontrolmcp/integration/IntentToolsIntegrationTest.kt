...
class IntentToolsIntegrationTest {
    ...
    @Test
    fun `send_intent activity with action calls dispatcher and returns success`() = runTest {
    ...
    @Test
    fun `send_intent broadcast with extras passes extras through`() = runTest {
        val deps = McpIntegrationTestHelper.createMockDependencies()
        coEvery {
            deps.intentDispatcher.sendIntent(
                type = "broadcast",
    ...
    @Test
    fun `send_intent with component passes component through`() = runTest {
        val deps = McpIntegrationTestHelper.createMockDependencies()
        coEvery {
            deps.intentDispatcher.sendIntent(
                type = "activity",
    ...
    @Test
    fun `send_intent with flags passes flags through`() = runTest {
        val deps = McpIntegrationTestHelper.createMockDependencies()
        coEvery {
            deps.intentDispatcher.sendIntent(
                type = "activity",
    ...
    @Test
    fun `send_intent with extras_types passes overrides through`() = runTest {
        val deps = McpIntegrationTestHelper.createMockDependencies()
        coEvery {
            deps.intentDispatcher.sendIntent(
                type = "activity",
    ...
    @Test
    fun `send_intent missing type returns error`() = runTest {
    ...
    @Test
    fun `send_intent invalid type returns error`() = runTest {
        McpIntegrationTestHelper.withTestApplication { client, _ ->
            val result = client.callTool(
                name = "android_send_intent",
    ...
    @Test
    fun `send_intent dispatcher failure returns error with message`() = runTest {
        val deps = McpIntegrationTestHelper.createMockDependencies()
        coEvery {
            deps.intentDispatcher.sendIntent(
                type = "activity",
    ...
    @Test
    fun `open_uri with package_name passes through`() = runTest {
        val deps = McpIntegrationTestHelper.createMockDependencies()
        coEvery {
    ...
    @Test
    fun `open_uri with mime_type passes through`() = runTest {
        val deps = McpIntegrationTestHelper.createMockDependencies()
        coEvery {
    ...
    @Test
    fun `open_uri missing uri returns error`() = runTest {
        McpIntegrationTestHelper.withTestApplication { client, _ ->
            val result = client.callTool(
                name = "android_open_uri",
                arguments = emptyMap(),
    ...
    @Test
    fun `open_uri dispatcher failure returns error with message`() = runTest {
        val deps = McpIntegrationTestHelper.createMockDependencies()
        coEvery {
    ...
    @Test
    fun `send_intent SecurityException returns sanitized error message`() = runTest {
        val deps = McpIntegrationTestHelper.createMockDependencies()
        coEvery {
            deps.intentDispatcher.sendIntent(
                type = "activity",
    ...
    @Test
    fun `send_intent IllegalStateException returns error`() = runTest {
        val deps = McpIntegrationTestHelper.createMockDependencies()
        coEvery {
            deps.intentDispatcher.sendIntent(
                type = "service",
