...
class NotificationProviderImplTest {
    ...
    private fun createMockSbn(
        ...
        val notification = mockk<Notification> {
            ...
            every { this@mockk.contentIntent } returns contentIntent
    ...
    private fun setServiceInstance(vararg sbns: StatusBarNotification) {
        ...
        every { service.getNotifications() } returns sbns
        McpNotificationListenerService.instance = service
    }
    ...
    inner class GetNotificationsTests {
        ...
        fun `getNotifications returns all notifications with correct data extraction`() = runTest {
        ...
        fun `getNotifications with packageName filter returns only matching`() = runTest {
        ...
        fun `getNotifications with limit returns at most N`() = runTest {
        ...
        fun `getNotifications sorted by postTime descending`() = runTest {
            val sbn1 = createMockSbn(key = "key1", postTime = 1000L)
        ...
        fun `getNotifications when service is null throws IllegalStateException`() = runTest {
            McpNotificationListenerService.instance = null
    ...
    inner class OpenNotificationTests {
        ...
        fun `openNotification finds notification by hash and sends contentIntent`() = runTest {
        ...
        fun `openNotification with unknown hash returns failure`() = runTest {
            setServiceInstance()
        ...
        fun `openNotification with no contentIntent returns failure`() = runTest {
        ...
        fun `openNotification with CanceledException returns failure`() = runTest {
    ...
    inner class DismissNotificationTests {
        ...
        fun `dismissNotification calls cancelNotification with correct key`() = runTest {
        ...
        @Test
        @DisplayName("with unknown hash returns failure")
        fun `dismissNotification with unknown hash returns failure`() = runTest {
            setServiceInstance()
        ...
        fun `dismissNotification with SecurityException returns failure`() = runTest {
            val sbn = createMockSbn(key = "key1")
            val service = mockk<McpNotificationListenerService>(relaxed = true)
            every { service.getNotifications() } returns arrayOf(sbn)
    ...
    inner class SnoozeNotificationTests {
        ...
        fun `snoozeNotification calls snoozeNotification with correct key and duration`() = runTest {
            val sbn = createMockSbn(key = "key1")
            val service = mockk<McpNotificationListenerService>(relaxed = true)
            every { service.getNotifications() } returns arrayOf(sbn)
            McpNotificationListenerService.instance = service
        ...
        @Test
        @DisplayName("with unknown hash returns failure")
        fun `snoozeNotification with unknown hash returns failure`() = runTest {
            setServiceInstance()
        ...
        @Test
        @DisplayName("with SecurityException returns failure")
        fun `snoozeNotification with SecurityException returns failure`() = runTest {
            val sbn = createMockSbn(key = "key1")
            val service = mockk<McpNotificationListenerService>(relaxed = true)
            every { service.getNotifications() } returns arrayOf(sbn)
    ...
    inner class ExecuteActionTests {
        ...
        fun `executeAction finds action by hash and sends PendingIntent`() = runTest {
            val actionPendingIntent = mockk<PendingIntent>(relaxed = true)
            val action = mockk<Notification.Action> {
                every { actionIntent } returns actionPendingIntent
                every { remoteInputs } returns null
                every { title } returns "Action1"
            }
        ...
        @Test
        @DisplayName("with unknown hash returns failure")
        fun `executeAction with unknown hash returns failure`() = runTest {
            setServiceInstance()
        ...
        fun `executeAction with null actionIntent returns failure`() = runTest {
            val action = mockk<Notification.Action> {
                every { actionIntent } returns null
                every { remoteInputs } returns null
                every { title } returns "Action1"
            }
            val sbn = createMockSbn(key = "key1", actions = arrayOf(action))
        ...
        @Test
        @DisplayName("with CanceledException returns failure")
        fun `executeAction with CanceledException returns failure`() = runTest {
            val actionPendingIntent = mockk<PendingIntent> {
                every { send() } throws PendingIntent.CanceledException()
            }
            val action = mockk<Notification.Action> {
                every { actionIntent } returns actionPendingIntent
    ...
    inner class ReplyToActionTests {
        ...
        fun `replyToAction sends PendingIntent with RemoteInput results`() = runTest {
            val actionPendingIntent = mockk<PendingIntent>(relaxed = true)
            val remoteInput = mockk<RemoteInput> {
                every { resultKey } returns "reply_key"
                every { isDataOnly } returns false
            }
            val action = mockk<Notification.Action> {
                every { actionIntent } returns actionPendingIntent
                every { remoteInputs } returns arrayOf(remoteInput)
                every { title } returns "Reply"
            }
            val sbn = createMockSbn(key = "key1", actions = arrayOf(action))
            setServiceInstance(sbn)
            val actionId = NotificationProviderImpl.computeActionHash("key1", 0)
        ...
        @Test
        @DisplayName("with unknown hash returns failure")
        fun `replyToAction with unknown hash returns failure`() = runTest {
            setServiceInstance()
        ...
        fun `replyToAction with no RemoteInput returns failure`() = runTest {
            val actionPendingIntent = mockk<PendingIntent>(relaxed = true)
            val action = mockk<Notification.Action> {
                every { actionIntent } returns actionPendingIntent
                every { remoteInputs } returns null
        ...
        @Test
        @DisplayName("with CanceledException returns failure")
        fun `replyToAction with CanceledException returns failure`() = runTest {
            val actionPendingIntent = mockk<PendingIntent> {
    ...
    inner class ToNotificationDataTests {
        ...
        fun `toNotificationData extracts title, text, bigText, subText from extras`() = runTest {
            val sbn = createMockSbn(
                key = "key1",
                title = "Title",
                text = "Text",
        ...
        fun `toNotificationData detects isOngoing from FLAG_ONGOING_EVENT`() = runTest {
        ...
        fun `toNotificationData maps actions with acceptsText from RemoteInput`() = runTest {
            val remoteInput = mockk<RemoteInput> {
                every { isDataOnly } returns false
            }
            val action = mockk<Notification.Action> {
        ...
        fun `toNotificationData uses packageName as appName fallback when getApplicationInfo fails`() = runTest {
            val extras = mockk<Bundle>(relaxed = true)
