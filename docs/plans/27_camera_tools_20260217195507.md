...
### Acceptance Criteria
- [ ] CameraX dependencies (camera-core, camera-camera2, camera-lifecycle, camera-video) are declared in the version catalog and build file
- [ ] `CAMERA` permission is declared in `AndroidManifest.xml`
- [ ] `RECORD_AUDIO` permission is declared in `AndroidManifest.xml`
- [ ] `<uses-feature android:name="android.hardware.camera" android:required="false" />` is declared (camera is optional, app works without it)
- [ ] `<uses-feature android:name="android.hardware.microphone" android:required="false" />` is declared
- [ ] Project builds without errors: `./gradlew assembleDebug`
- [ ] Linting passes: `./gradlew ktlintCheck detekt`
